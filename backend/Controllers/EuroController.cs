...
namespace election_calculator_backend.Controllers
{
    ...
    {
        [HttpGet("load")]
        public async Task<ActionResult<List<object>>> GetEuroSummaries()
        {
            var euroJson = await System.IO.File.ReadAllTextAsync(
                Path.Combine(Directory.GetCurrentDirectory(), "Data", "European_Parliament.json")
            );
            var euroDistricts = JsonSerializer.Deserialize<List<EuroDistrict>>(
                euroJson,
            ...
            var filteredTerritories = territories.Where(t => t.EuroparlamentID != null).ToList();

            var result = euroDistricts
                .Select(euroDistrict =>
                {
                    var relatedCounties = counties
                        .Where(c => c.EuroparlamentID == euroDistrict.Id)
                        .ToList();
                    var relatedTerritories = filteredTerritories
                        .Where(t => t.EuroparlamentID == euroDistrict.Id)
                        .ToList();
